...
public:
    ...
    string generateId_ForOrder(int driverId, const string& tujuan, const string& customerName, const string& plateNumber);
    void add_DataSupir();
    void edit_SupirData();
    void Erase_Supir();
...
DriverList::DriverList() : head(nullptr), tail(nullptr), idCounter(0), headOrder(nullptr), tailOrder(nullptr) {}

Order* DriverList::getHeadOrder() {
    return headOrder;
}

void DriverList::displayCompletedOrders(){
...
void DriverList::addOrderToList(Order newOrder){
...
void DriverList::addOrder(Order newOrder){
    Order *newNode = new Order;
    newNode->id = newOrder.id;
    newNode->nama = newOrder.nama;
    newNode->supir = newOrder.supir;
...
string DriverList::generateId_ForDriver(DataSupir supir) {
        string id = "";
...
void DriverList::displaySupir(Node *current) {
...
void DriverList::displayListAdmin() {
        int index = 0;
        Node *current = head;
...
void DriverList::edit_SupirData() {
        string idSupir;
...
void DriverList::displayListUser() {
    Node *here = head;
    queue<Order> orderQueue;
    string idSupir;
