...
void Perpustakaan::merge(int L, int M, int R) {
    ...
    Buku *left = new Buku[n1];
    Buku *right = new Buku[n2];
    ...
    // Merge the temp arrays back into table[L..R]
    int i = 0, j = 0, k = L;
    while (i < n1 && j < n2) {
        if (left[i].judul <= right[j].judul) {
            table[k] = left[i];
            i++;
        } else {
            table[k] = right[j];
...
int Perpustakaan::CariBukuLinear(const string& searchBook) {
    for (int i = 0; i < size; ++i) {
        if (table[i].judul == searchBook) {
            return i; // Buku ditemukan pada indeks i
        }
    }
